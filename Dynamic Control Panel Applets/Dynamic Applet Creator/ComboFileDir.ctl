VERSION 5.00
Begin VB.UserControl ComboFileDir 
   BackColor       =   &H8000000E&
   ClientHeight    =   495
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   3630
   PropertyPages   =   "ComboFileDir.ctx":0000
   ScaleHeight     =   33
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   242
End
Attribute VB_Name = "ComboFileDir"
...
Private m_StartingNodeBold As Boolean
Private Const m_StartingNodeBold_ByDef As Boolean = False
Private m_IncludeFiles As Boolean, m_FileSpec As String
Private Const m_IncludeFiles_ByDef As Boolean = False
...
Attribute zzSubclass_Proc.VB_MemberFlags = "40"
    ...
    Select Case uMsg
        ...
        Case BFFM_SELCHANGED
            ...
            If SHGetPidlInfo(ByVal lParam, 0&, SHFI, Len(SHFI), SHGFI_ICON Or SHGFI_SMALLICON Or SHGFI_DISPLAYNAME Or SHGFI_PIDL) <> 0 Then
                lstrDisplayName = Left(SHFI.szDisplayName, InStr(SHFI.szDisplayName, vbNullChar) - 1)
                If UCase(Right(lstrDisplayName, 3)) = FileSpec Then
                    Call yPaint(True, False, True, lstrDisplayName, SHFI.hIcon)
...
Private Sub yShowFolderBrowse()
    ...
    With BI
        ...
        If m_IncludeFiles Then .ulFlags = .ulFlags Or BIF_BROWSEINCLUDEFILES
...
Public Property Let IncludeFiles(ByVal Value As Boolean)
